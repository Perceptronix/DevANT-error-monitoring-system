...
export function PipelineVisualizer({ state }: PipelineVisualizerProps) {
  ...
  // When pipeline completes, keep the last step (alert preview) open
  useEffect(() => {
    if (state?.status === 'completed' && !pipelineComplete) {
      setPipelineComplete(true)
      // Keep the last step expanded
  ...
  if (!state) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center">
        ...
        <p className="text-lg font-medium text-foreground mb-2">Ready to analyze errors</p>
        <p className="text-sm text-muted-foreground max-w-md">
          The demo will process 20 sample errors through clustering, context search, and severity analysis.
        </p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {state.steps.map((step, idx) => {
          ...
          // Auto-collapse all steps except the last one when pipeline is running
