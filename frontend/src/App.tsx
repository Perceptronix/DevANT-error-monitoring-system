...
interface Config {
  ...
  // Enhanced config from updated backend
  data_source?: {
    active: string
    type: string
  }
  integrations?: {
    linear: { enabled: boolean; configured: boolean; mode: string }
  ...
  const runPipeline = useCallback(() => {
    ...
    send({
      ...
      config: {
        ...
        include_extended_samples: false,
      },
    })
  }, [send])
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <a href="https://airweave.ai" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                ...
                <div>
                  <h1 className="text-lg font-semibold">Error Monitoring Agent</h1>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {config.data_source && (
                    <Badge variant="secondary" className="font-normal">
                      {config.data_source.active}
                    </Badge>
                  )}
                  {config.integrations?.linear && (
                    <Badge 
                      variant={config.integrations.linear.mode === 'live' ? 'default' : 'outline'}
                      className={config.integrations.linear.mode === 'preview' ? 'text-amber-600 border-amber-600' : ''}
                    >
                      Linear {config.integrations.linear.mode === 'live' ? '✓' : '(preview)'}
                    </Badge>
                  )}
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Watch raw errors transform into actionable, context-rich alerts
            </p>
          </div>
        ...
        {!pipelineState && (
          <div className="mt-12 grid md:grid-cols-3 gap-6">
            <InfoCard
              ...
              description="Semantic clustering groups similar errors by root cause. Get one actionable alert instead of dozens of duplicates."
              icon="🎯"
            />
            <InfoCard
              title="Instant Context"
              description="Airweave automatically finds related code, tickets, and Slack threads so you understand the issue immediately."
              icon="🔍"
            />
            <InfoCard
              title="Smart Prioritization"
              description="Critical and regression issues alert immediately. Known issues are tracked without flooding your channels."
              icon="⚡"
            />
          </div>
        )}
        ...
        {/* Preview mode notice */}
        {!pipelineState && config?.integrations && (
          (config.integrations.linear?.mode === 'preview' || config.integrations.slack?.mode === 'preview') && (
            <div className="mt-6 p-4 rounded-lg border border-border bg-muted/50">
              <p className="text-sm text-muted-foreground">
                <strong className="text-foreground">Demo Mode:</strong> Showing preview of what would be sent to Linear and Slack. 
                Configure API keys to enable real integrations.
              </p>
            </div>
          )
        )}
